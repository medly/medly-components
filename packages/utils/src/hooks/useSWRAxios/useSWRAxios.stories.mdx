...
This react hook can be used to call the api using `swr` and `axios`. `swr` helps to cache the data. Chekout more about `swr` over [here](https://swr.vercel.app/)
...
const Demo: React.SFC = React.memo(() => {
    const { data, isValidating, error } = useSWRAxios<{ name: string }, { message: string }>({ url: 'https://somedummyurls.com/data' });
