...
You can use the `useAxios` to call the API in any react component that exposed, `isLoading`, `data` and  `error` state so that you don't have to create it multiple times in your components.
...
You can set the global config for Axios like given below. To know more about Axios click [here](https://github.com/axios/axios#global-axios-defaults).
