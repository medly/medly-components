...
type CustomPhoneFieldProps = { value: string; name: string; onChange: (name: string, value: string) => void; onRemoveField: (name: string) => void; };
const CustomPhoneField: FC<CustomPhoneFieldProps> = ({ value, name, onRemoveField, onChange }) => {
    return (
        <div style={{ display: 'flex', alignItems: 'center' }}>
            ...
            <Button variant='flat' size='XS' onClick={() => onRemoveField(name)}>Remove</Button>
        </div>
    );
};
