...
<Meta title="Forms with custom fields" parameters={{ jest: ['Form.test.tsx', 'Actions/Actions.test.tsx'] }} />
...
Try clicking the '+ Add new phone number' button.

<Preview withToolbar>
    <CustomFieldForm />
</Preview>

This is the initial schema which would be converted to a `formSchema` object.
```tsx
const defaultFields = [
...
export const AddPhoneNumber: FC<AddPhoneNumberProps> = ({ fields, setFields }) => {
    ...
    return (
        <div>
            <Button onClick={addPhoneNumberField} variant='flat' size='XS' disabled={disabled}>+ Add new phone number</Button>
        </div>
    );
};
```
