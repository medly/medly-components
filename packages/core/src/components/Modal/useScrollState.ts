...
export const useScrollState = ({ ref, scrollState, dispatch }: UseScrollStateProps): ((e: React.UIEvent<HTMLDivElement>) => void) => {
    useEffect(() => {
        ...
        ref.current && dispatch({ type: 'scrolledToBottom', value: (element.getBoundingClientRect().height) === element.scrollHeight });
    ...
    const handleScroll = (e: UIEvent<HTMLDivElement>) => {
        ...
        const element = e.target as HTMLElement,
            ...
            isScrolledToBottom = Math.ceil(element.scrollTop + (element.getBoundingClientRect().height)) === element.scrollHeight;
