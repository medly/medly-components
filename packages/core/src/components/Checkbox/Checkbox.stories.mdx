...
1. This component will call the validator prop function with latest value on blur, invalid & change event. If you havn't passed the validator function then it will show default html5 error validation meessage if present.
...
<Preview withToolbar>
    <FormWithCheckbox validator={val => (!val ? 'Please accept the terms & conditions to continue.' : '')} />
</Preview>
