import { optionsArray, boxSize, ThemeInterface } from './SearchBox.stories.tsx';
import { Preview, Story, Meta, Props } from '@storybook/addon-docs/blocks';
import * as stories from './SearchBox.stories';
import { select } from '@storybook/addon-knobs';
import { SearchBox } from './SearchBox';

<Meta title="Core" component={SearchBox} />

# SearchBox

### Add a auto suggest text box

<Preview withToolbar>
    <Story name="SearchBox">{
        () => {
            const onGetOptions = () => optionsArray;
        return (
            <SearchBox placeholder="Search" getOptions={onGetOptions} boxSize={select('boxSize', boxSize, 'S')} />
            )
        }
    }</Story>
</Preview>


### Props

<Props of={SearchBox} />

##### Small search text box

<Preview>
    <SearchBox boxSize='S' />
</Preview>

##### Medium search text box

<Preview>
     <SearchBox boxSize='M' />
</Preview>

### SearchBox Theme

<Props of={ThemeInterface} />