...
MultiSelect with creatable options allows user to create new options apart from what are already present. This feature requires `setValues`and `inputValue` prop apart from the `isCreatable` flag.

<Preview withToolbar>
    <Story name="WithCreatableOptions">
        {() => {
            const [values, setValues] = useState(['lorem pharmacy', 'a pharmacy']);
            const [inputValue, setInputValue] = useState('lorem pharmacy, a pharmacy');
            return (
                <MultiSelect
