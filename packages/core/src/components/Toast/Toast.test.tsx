...
describe('Toast', () => {
    ...
    it('should render properly with fullWidth prop', () => {
        const { container } = render(<Toast variant="success" message="Info message" fullWidth />);
        expect(container).toMatchSnapshot();
    });
    ...
    it('should render custom icons', () => {
        const { container } = render(<Toast variant="success" icon={HomeIcon} />);
        expect(container).toMatchSnapshot();
    });

    it('should render action button', () => {
        const mockOnButtonClick = jest.fn(),
            { container, getByRole } = render(<Toast variant="success" action={{ label: 'Action 1', onClick: mockOnButtonClick }} />);
