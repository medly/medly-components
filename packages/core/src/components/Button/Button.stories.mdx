...
To use custom colors for `button` component you can provide the custom `theme` to `button` component. You can use the `updateNestedValue` function available in `@medly-components/utils` to update the nested values in the `defaultTheme` object. Check out the `Theme` section to find out which all properties are mandatory in different variants.
