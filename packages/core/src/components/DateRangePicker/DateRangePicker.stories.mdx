...
To perform an action after the calendar popup closes, pass a function using the `onPopupClose` prop.
Use the `onChange` prop to make dateRangePicker a controlled component, and the `onPopupClose` prop to handle actions (e.g. API calls) that you want to perform after date selection.
