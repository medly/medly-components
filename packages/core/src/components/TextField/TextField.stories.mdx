...
To add the masking feature, you need to provide the `mask` prop. The feature currently supports only numeric input, so at the moment, the user can only type numbers.
