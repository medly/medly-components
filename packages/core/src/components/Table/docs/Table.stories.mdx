            ...
            return (
                <DummyWrapper>
                    ...
                    <Table
                        ...
                        enableActionBar={boolean('Enable action bar', true)}
                        actions={Actions}
...
To show action bar you have to pass `enableActionBar` as `true` and pass `actions` in below format
...
To show pagination you have to pass `enablePagination` as `true` and pass `totalItems`, `itemsPerPage` & `onPageChange` props. `onPageChange` callback function will recieve object with these values `{ activePage?: number; sortField?: string; sortOrder?: SortOrder }`
