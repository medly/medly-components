...
To show the Row Grouping, you have to set `groupBy` prop with key name you want to group your data. And your `data` should be in the below format:
...
To add a Row Selection feature to your table you have to complete the following two steps:
...
To display Minimap you must pass `withMinimap` as `true` and it will be visible when there is a horizontal scroll available in the table.

### Pagination

To display Pagination you must pass `withPagination` as `true` and pass `totalItems`, `itemsPerPage` & `onPageChange` props. The `onPageChange` callback function will recieve object with these values `{ activePage?: number; sortField?: string; sortOrder?: SortOrder }`
