...
<Preview withToolbar>
    <Story name="WithoutColumnHide">{stories.Basic()}</Story>
</Preview>

### With column hidden feature

To use the column hidden feature use `Table.SelectableColumns` component and save the column config in state variable and pass the same column config to `Table` and `Table.SelectableColumns` component.
...
export const WithColumnHide = () => {
    ...
    return (
        <>
            <Table.SelectableColumns columns={columnConfig} fieldToChange="hidden" onChange={setColumnConfig} />
