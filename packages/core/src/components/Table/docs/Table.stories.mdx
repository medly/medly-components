...
These are the default values, you can also override the default key values by passing the keyBindings object to the `keyBindings` prop. Be sure to match the key values to the HTML spec. You can view the key values [here](https://developer.mozilla.org/en-US/docs/Web/API/KeyboardEvent/key/Key_Values).
