...
You can navigate rows in table using `Up Arrow` and `Down Arrow` key. To select a row you can use `Space` key (default browser value) whereas you can expand a row using the `Right Arrow` key and collapse a row using the `Left Arrow` key.
These are the default values, you can also override the default key values (except the select `Space` key) by passing the keyBindings object to the `keyBindings` prop. Be sure to match the key values to the HTML spec. You can view the key values [here](https://keycode.info/).
