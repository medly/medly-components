...
1. For now `ChackboxGroup` will call the `validator` prop function with latest values on `blur` & `change` event. If you havn't passed the validator function then it will show default error message if you pass `required` prop.

> **Note**: Right now validattion message will not apear on `submit` event. We are working on it.
