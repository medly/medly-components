...
> Note: If you're using Webpack 5, you'll need to add the following webpack config and need to add the following dependencies: `yarn add util assert buffer process`.
...
> Note: If you're using jest for testing, you'll need to mock the `@medly-components/markdown`. This is due to `react-markdown` using some untranspiled modules. Since all files inside node_modules are not transformed by default, Jest will not understand the code in these modules, resulting in syntax errors.
...
The above markdown content will get rendered as followed
